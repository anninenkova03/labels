...
class LabelInjector {
        ...
        for (List<Integer> chunk : transformationsInChunks) {
            if (chunk.size() == 1) {
                ...
                if (transformationType == 8) {
                    ...
                    label = new TextTransformationDecorator(label, getTransformation(chunk, input));
                }
            } else {
                label = new TextTransformationDecorator(label, getTransformation(chunk, input));
            }
    ...
    private TextTransformation getTransformation(List<Integer> chunk, Scanner input) {
